...
import io # Para ler a imagem da memória
from src.config import settings # Importa as configurações para saber o tópico
...
class FirmwareSimulado:
    def __init__(self, porta_serial):
        ...
        self.mapa_surface = None # Variável para guardar a imagem do mapa

        # --- Conexão Serial (como antes) ---
        try:
        ...
        # --- NOVA: Conexão com o Broker MQTT ---
        try:
            print("Firmware conectando ao Broker MQTT para receber mapas...")
            self.mqtt_client = mqtt.Client()
            self.mqtt_client.on_message = self._on_mqtt_map_message # Define a função de callback
            self.mqtt_client.connect(settings.mqtt_broker_host, settings.mqtt_broker_port, 60)
            self.mqtt_client.subscribe(settings.mqtt_topico_mapa) # Se inscreve no tópico do mapa
            self.mqtt_client.loop_start() # Inicia o loop do cliente em uma thread separada
            print("Firmware conectado ao MQTT com sucesso!")
        except Exception as e:
            print(f"AVISO: Não foi possível conectar ao MQTT para receber mapas. Visualização do mapa ficará desativada. Erro: {e}")

    # --- NOVA: Função de Callback do MQTT ---
    def _on_mqtt_map_message(self, client, userdata, msg):
        """Essa função é chamada automaticamente quando uma mensagem chega no tópico do mapa."""
        try:
            # Decodifica a string Base64 para dados de imagem binários
            image_data = base64.b64decode(msg.payload)
            # Cria um "arquivo" em memória com esses dados
            image_stream = io.BytesIO(image_data)
            # Carrega a imagem do "arquivo" em memória para uma superfície do Pygame
            self.mapa_surface = pygame.image.load(image_stream).convert()
            print("FIRMWARE (VISUAL): Novo mapa recebido e atualizado na tela!")
        except Exception as e:
            print(f"FIRMWARE (VISUAL): Erro ao processar imagem do mapa recebida via MQTT: {e}")

    # ... (toda a lógica do firmware: _chassi_avancar, _fazer_scan, etc. não muda) ...
    ...
    def _fazer_scan(self):
        print("FIRMWARE (SIM): Iniciando rotina de scan...")
        ...
        for angulo_graus in range(0, 181, 10):
            ...
            time.sleep(0.04)
        print("FIRMWARE (SIM): Scan concluído.")
    ...
    def loop_principal(self):
        ...
        while rodando:
            ...
            # ATUALIZADO: Passa o mapa recebido via MQTT para a função de desenhar
