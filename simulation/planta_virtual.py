...
# Aumentamos a largura para a tela dividida
LARGURA, ALTURA = 1200, 600 
COR_FUNDO = (20, 20, 20)
COR_PAREDE = (200, 200, 200)
COR_ROBO = (0, 200, 100)
COR_SENSOR = (255, 50, 50)
ESCALA_VISUALIZACAO = 2.5 # Ajustei a escala para caber melhor

# Mapa simulado (sem alterações)
PAREDES_CM = [
    # Bordas externas
    ((10, 10), (260, 10)),
    ((260, 10), (260, 160)),
...
class Planta:
    ...
    def desenhar(self, pos_robo_cm, angulo_robo_rad, pontos_scan_cm, mapa_surface):
        ...
        # Desenha o robô (simplificado para um círculo para clareza)
        ...
        for angulo_rel_graus, dist_cm in pontos_scan_cm:
            if dist_cm > 0 and dist_cm < 200: # Não desenha raios infinitos
        ...
        # --- LADO DIREITO: DESENHA O MAPA DO ROBÔ ---
        if mapa_surface:
            # Redimensiona o mapa gerado para caber na área de visualização
    ...
    # A função calcular_distancia não muda
    def calcular_distancia(self, pos_robo_cm, angulo_scan_rad):
        ...
        for parede in PAREDES_CM:
            ...
            if 0 < t < 1 and u > 0 and u < 1: # Correção para garantir interseção no segmento
                px, py = x1 + t * (x2 - x1), y1 + t * (y2 - y1)
                dist = math.hypot(px - x1, py - y1)
                if dist < dist_min:
                    dist_min = dist
        return int(dist_min) if dist_min != float('inf') else 200 # Retorna 200 se não houver colisão